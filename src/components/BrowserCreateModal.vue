...
function batchCreate(){
  const count = window.prompt('请输入要创建的浏览器数量：', '5');
  if (!count || isNaN(Number(count)) || Number(count) <= 0) {
    return;
  }
  const num = Number(count);
  const batchCount = window.prompt(`确认创建 ${num} 个浏览器？\n请输入 'confirm' 确认：`, '');
  if (batchCount !== 'confirm') {
    return;
  }

  for (let i = 0; i < num; i++) {
    const batchForm = JSON.parse(JSON.stringify(form));
    // 如果名称为空，自动生成默认名称
    if (!batchForm.name.trim()) {
      batchForm.name = generateDefaultName();
    } else {
      // 如果有名称，添加序号
      batchForm.name = `${batchForm.name}-${i + 1}`;
    }
    emit('submit', batchForm);
  }
  close();
}
</script>
  ...
  <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center">
    ...
    <div class="relative w-[720px] max-h-[80vh] flex flex-col rounded-lg p-4" :class="isDark ? 'bg-[#111a22] text-white' : 'bg-white text-[#0d141b]'">
      ...
      <div class="mt-4 flex justify-between gap-2">
        <button class="h-9 px-4 rounded" :class="isDark ? 'bg-[#233648] text-white' : 'bg-[#e7edf3] text-[#0d141b]'" @click="batchCreate">批量创建</button>
